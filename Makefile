...
redo-db: create-db silent_run migrate load_initial_data run
...
create-db:
	docker-compose exec db mysql -u image_processing_user -p -e "CREATE DATABASE image_processing CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
...
drop-db: ## LOCAL - Drop the database
	...
	docker-compose exec db dropdb -U image_processing image_processing
